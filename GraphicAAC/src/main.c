...
void state_changed_cb(tts_h tts, tts_state_e previous, tts_state_e current, void* user_data)
{
	if (TTS_STATE_READY == current && TTS_STATE_CREATED == previous) {
		...
		if(op == 0) {
			...
			/*코로나 부분*/
			if(tts_num==0) {
				text = "중국 우한에서 발생한 폐렴은 신종 코로나 바이러스인 2019년 12월 처음 인체 감염이 확인됐다는 의미에서 '코로나19'로 불리웁니다. 중국 우한에서 발생한 바이러스는 기존 코로나 바이러스와는 성질이 달라 신종 코로나 바이러스로 분류됐습니다.";
			} else if (tts_num==1) {
				text = "국제보건기구(WHO)에 따르면 우한에서 발생한 새로운 코로나 바이러스의 확산은 동물에서 비롯된 것으로 보고 있습니다. 신종 코로나 바이러스는 새로운 바이러스이기 때문에 감염 경로는 아직 명확하게 밝혀지지 않은 상태입니다.";
			} else if (tts_num==2) {
				text = "코로나19에 감염되면 2~3일에서 최장 2주 정도 잠복기를 거쳤다가 다양한 증상이 나타납니다. 37.5도 이상의 고열, 기침, 인후통, 가래, 근육통, 두통, 호흡곤란, 폐렴 등의 증상이 발생합니다. 폐 손상에 따른 호흡부전으로 심하면 사망에 이를 수도 있습니다.";
			} else if (tts_num==3) {
				text = "현재까지 코로나19 치료법은 없습니다. 증상에 따른 약제를 사용하고 에이즈 치료제나 항말라리아 제제를 이용하여 치료한 사례가 있으며 호흡곤란을 호소하는 경우 산소 마스크를 적용할 수 있습니다.";
			} else if (tts_num==4) {
				text = "어제 기준으로 국내 코로나 확진자는 9478명, 서울은 390명, 마포구는 14명 확진받았습니다.";
			} else if (tts_num==5) {
				text = "철저한 손씻기가 가장 중요한 감염병 예방법입니다. 사람의 분비물은 직접 튀는 경우보다, 그 분비물이 어딘가에 묻었는데 손으로 만져서 몸으로 들어올 확률이 더 높다고 알려져 있습니다. 흐르는 물에 비누를 이용하여 30초 이상 손을 씻으면 바이러스는 거의 다 죽거나 힘이 약해져 감염을 일으키기 힘들다고 합니다.";
			} else if (tts_num==6) {
				text = "마스크는 감염자의 분비물이 공기 중으로 날아가지 않게하고, 공기 중의 바이러스가 호흡기를 통해 들어오지 못하도록 막아주는 역할을 합니다. 기침 시에는 분비물이 손이나 공기 중에 뿌려지지 않도록 옷소매에 하는 것이 좋습니다. 또 호흡기 증상이 있는 사람과의 밀접한 접촉을 피하고 사람 많이 모인 곳에는 가지 않아야 하겠습니다.";
			} else if (tts_num==7) {
				text = "만약 코로나19에 걸린것 같으면 마포구 보건소 선별진료소에 가서 검사 받아야합니다.";
			} else if (tts_num==8) {
				text = "선별진료소에 가면 상담과 열을 측정하고 가래검사, 입 면봉검사,  코면봉검사를 하게됩니다, 결과는 다음날 나옵니다.";
			} else if (tts_num==9) {
				text = "코로나19 설명은 끝났습니다. 혹시 질문있는 사람 있어요?";
			}
		...
		tts_get_speed_range(ad->tts, &min, &normal, &max);
		dlog_print(DLOG_INFO, LOG_TAG, "min : %d, normal : %d, max : %d", min, normal, max);

		ret = tts_add_text(ad->tts, text, language, voice_type, min, &utt_id);
